 ...
 * subscriptions. A user maps to a single ePayco customer; cards are tokens
 * attached to it. The browser never sees a raw PAN after tokenization.
 */
...
export async function resolveCardAndCustomer(
	...
	if (opts.savedTokenId) {
		if (!customer) {
			throw new ConvexError({ message: "No saved profile found for this card." });
		}
		const tokens = await epayco.getLocalTokens(ctx, { userId });
		const match = tokens.find(
			(t: { epaycoTokenId: string }) => t.epaycoTokenId === opts.savedTokenId,
		);
		if (!match) {
			throw new ConvexError({ message: "That saved card is no longer available." });
		}
		return {
			tokenCard: opts.savedTokenId,
			customerId: customer.epaycoCustomerId,
		};
	}

	if (!opts.card) {
		throw new ConvexError({ message: "Provide card details or pick a saved card." });
	}
	...
	if (!created) {
		throw new ConvexError({ message: "Could not create your payment profile." });
	}
